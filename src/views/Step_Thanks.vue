...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // As our SetupIntent has already succeeded, we can't cancel it...
      async onCancelOrder() {

        // // If no Customer id found, then just bump back to Start page.
        // if (this.$store.state.setupIntentId == '') {
        //   console.log('No PaymentIntent found. Not able to Cancel order. Please go to Start page.')
        //   this.tmpStripeFailChip = true
        //   this.tmpStripeSuccessChip = false
        //   return
        // }
        ...
        //   "setupIntentId": this.$store.state.setupIntentId
        // }
        // console.log('postData for Cancel', postData)

        // const response = await fetch(
        //   'https://gateway.flyzermatt.com/cancel-order', {
        //     method: 'POST',
        ...
        //   console.log('Fatal Error => Not able to connect to Stripe: ', response.error, response.payment_intent)
        //   this.tmpStripeFailChip = true
        //   this.tmpStripeSuccessChip = false
        //   return
        // }

        // console.log("Response from CANCEL Call: ", response)

        // let returnedJSON = await response.json()

        // console.log("JSON from CANCEL Call: ", returnedJSON)

        // // Also need to check if we got response.result == 'success'
        // if (returnedJSON.result === 'success') {
        //   console.log('Order Cancelled! Check Stripe for details.')
        //   this.tmpStripeFailChip = false
        //   this.tmpStripeSuccessChip = true
        //   this.stripeCancelOrderBtnDisabled = true
        // } else {
        //   // Something went wrong, not getting happiness back from Server.
        //   console.log("Server call to CANCEL payment didn't succeed. Reason: ", returnedJSON.result)
      ...
      // Just a quick test method to complete a Stripe order.
      async onCompleteOrder() {

        alert("This no longer works - complete on Tommy's system...")
        

        // If no Customer id found, then just bump back to Start page.
        if (this.$store.state.custClientId == '') {
          console.log('No Customer ID found. Please go to Start page.')
          this.tmpStripeFailChip = true
          this.tmpStripeSuccessChip = false
          return
        }
        ...
        const postData = { 
          ...
          "amount": '22000',  // TODO: need real value here. 22000 is equiv to 220.00 CHF (sending cents). Needs to come from Tommy's backend UI.
          "description": "Called Order Completed from Tommy's API",   // TODO: This would be a good place to put any order completed notes/infos.
        }
        console.log('postData', postData)


        const response = await fetch(
          'https://gateway.flyzermatt.com/complete-order', {
            method: 'POST',
            body: JSON.stringify(postData),
            headers: {
              'Content-Type': 'application/json'
        ...
        console.log("JSON from Complete Call: ", returnedJSON)

        // Also need to check if we got response.result == 'success'
        if (returnedJSON.result === 'success') {
          console.log('Order Completed! Check Stripe for details.')
          this.tmpStripeFailChip = false
          this.tmpStripeSuccessChip = true
          this.stripeCompleteOrderBtnDisabled = true
        } else {
          // Something went wrong, not getting happiness back from Server.
          console.log("Server call to complete payment didn't succeed. Reason: ", returnedJSON.result)
